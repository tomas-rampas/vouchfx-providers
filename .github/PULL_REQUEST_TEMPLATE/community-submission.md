...
- [ ] **Namespace hygiene** — the provider does **not** declare `Vouchfx.Engine.*` or
      `Vouchfx.Steps.*` (reserved; the engine refuses them at startup). The
      `Vouchfx.Community.<Name>` convention is recommended.
...
- [ ] **Builds standalone** — the project and its tests restore from NuGet.org against the pinned
      `Vouchfx.Sdk` version (see `CONTRIBUTING.md` "Building against the SDK"); no engine-repo
...
- [ ] **Conformance** — the SDK conformance harness (`Vouchfx.Sdk.Testing`) passes locally;
