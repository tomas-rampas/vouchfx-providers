...
One neighbouring thing this registry is *not*: the [vouchfx-samples](https://tomas-rampas.github.io/vouchfx-samples/) project hosts sample *applications* (systems under test in C#, Python and Java) with complete `.e2e.yaml` suites demonstrating how the engine tests them. It also includes the canonical worked example of consuming a community provider from this registry: [`ledger-jsonrpc`](https://tomas-rampas.github.io/vouchfx-samples/samples/ledger-jsonrpc/README.html), which depends on `Vouchfx.Community.JsonRpc` from NuGet.
